...
class Pyoro2(Pyoro):
    ...
    def enable_move_left(self):
        ...
        if self.shoot_sprite_id:
            self.shoot_sprite_id = 0
            self.level.remove_action_delay((self, "update_shoot_sprite_id"))
    ...
    def update(self, delta_time):
        ...
        if self.shoot_sprite_id == 1:
            self.level.create_action_delay(
                (self, "update_shoot_sprite_id"),
                PYORO_SHOOT_SPRITE_DURATION,
                self.update_shoot_sprite_id,
            )
    ...
    def update_sprite(self):
        ...
        if self.dead:
            self.current_image_name = f"pyoro_{style_type}_die_{self.direction}.png"
        elif self.shoot_sprite_id:
            self.current_image_name = (
                f"pyoro_{style_type}_shoot_{shoot_type}_{self.direction}.png"
            )
        elif self.notch:
    ...
    def enable_capacity(self):
        ...
        for entity in self.level.entities:
            if self.is_shooting_entity(entity):
                if isinstance(entity, Bean):
                    ...
                    entity.cut()
                    if self.direction == 1:
                        entity.set_right_wind()
                    else:
                        entity.set_left_wind()
        ...
        for bean_pos in bean_coords:
            self.level.spawn_score(score, bean_pos)
    ...
    def is_shooting_entity(self, entity):
        ...
        return (
            self.pos[1] - entity.pos[1] + entity.size[1] >= distance - entity.size[0]
        ) and (
            self.pos[1] - entity.pos[1] - entity.size[1] <= distance + entity.size[0]
        )
